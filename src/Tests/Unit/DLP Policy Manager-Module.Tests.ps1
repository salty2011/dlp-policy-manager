...
Describe 'Module Tests' -Tag Unit {
    Context "Module Tests" {
        ...
        It 'root module DLP Policy Manager.psm1 should exist' {
            $PathToModule | Should -Exist
            $? | Should -BeTrue
        } #psm1Exists
        It 'manifest should contain DLP Policy Manager.psm1' {
            $PathToManifest |
            Should -FileContentMatchExactly "DLP Policy Manager.psm1"
        } #validPSM1
