function Test-ModuleRequirements {
    ...
    # Function to test if a module is installed with the correct version
    ...
    function Install-RequiredModule {
        ...
        if (-not (Test-Module -ModuleName $ModuleName -VersionOperator $VersionOperator -RequiredVersion $RequiredVersion)) {
    ...
    $requirements = Import-Requirements -FilePath $RequirementsFilePath

    # Test modules and optionally install/update them
    foreach ($moduleName in $requirements.Keys) {
        ...
        if (-not (Test-Module -ModuleName $moduleName -VersionOperator $versionOperator -RequiredVersion $requiredVersion)) {
            ...
            if ($InstallMissing.IsPresent) {
                Install-RequiredModule -ModuleName $moduleName -VersionOperator $versionOperator -RequiredVersion $requiredVersion
            }
        } else {
            Write-Verbose "Module $moduleName with version $versionOperator $requiredVersion is already installed."
        }
    ...
    if ($InstallMissing.IsPresent) {
        Write-Verbose "All required modules are tested and installed/updated as necessary."
    } else {
        Write-Verbose "Module test completed. Use -InstallMissing to install missing dependencies."
    }
}